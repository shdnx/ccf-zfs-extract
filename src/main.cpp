...
struct BlockTreeNode {
  explicit BlockTreeNode(size_t size) : m_size{size} { ASSERT0(size > 0); }
...
private:
  ...
  friend struct BlockTreeInnerNode;
};

struct BlockTreeInnerNode : BlockTreeNode {
  explicit BlockTreeInnerNode(size_t size, size_t level)
      : BlockTreeNode{size}, m_level{level} {
    ASSERT0(level > 0);
  ...
  BlockTreeNode *child(size_t index) {
  ...
  BlockTreeNode *readChild(ZPool &zpool, size_t index, size_t dva = 0) {
    if (BlockTreeNode *result = child(index))
      return result;
    ...
    BlockTreeNode *node;
    if (m_level == 1)
      node = new BlockTreeNode{bp.getLogicalSize()};
    else
      node = new BlockTreeInnerNode{bp.getLogicalSize(), m_level - 1};

    if (!node)
      return nullptr;

    if (!zpool.readRawData(bp, dva, OUT & node->m_data))
      return nullptr;

    if (m_children.empty())
      m_children.resize(numChildren());

    return m_children[index] = node;
  }

private:
  size_t                       m_level;
  std::vector<BlockTreeNode *> m_children; // TODO: could reuse m_data
};
